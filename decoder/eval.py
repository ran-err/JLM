...
parser.add_argument("--experiment_id", "-e", type=int, default=2, help="experiment id to eval")
# parser.add_argument("--eval_size", "-es", type=int, default=100, help="Number of sentences to evaluate")
parser.add_argument("--eval_size", "-es", type=int, default=200, help="Number of sentences to evaluate")
        ...
        with open(os.path.join(data_path, 'test.txt'), 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                if not has_oov(tokens):
                    readings = ''.join([x.split('/')[1] if x != '' and x.split('/')[1] != '' else x.split('/')[0] for x in tokens])
